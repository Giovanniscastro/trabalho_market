...
class TestClientes(TestCase):
    def test_positivo_insere_cliente(self):
        cliente = Cliente(cod_cliente=None,nome="test", sobrenome="test", cpf="test",
                          email="test", senha="test", endereco="test", telefone="test")
        print(cliente)
    ...
    def test_positivo_altera_cliente(self):
        ...
        print(cliente)
        cliente_db = ClienteDb()
        result = cliente_db.altera_cliente_db(7, cliente)
        self.assertEqual("Sucesso", result)
